...
router.get('/:id/powers', block_access.isLoggedIn, function (req, res) {
    models.E_user.findOne({where: {id: req.session.passport.user.id}, include: [{model: models.E_power, as: 'r_power', where: {f_label: {$ne: 'Vote'}},required:false}]}).then(function (user) {
        if (user) {
