...
router.post('/datalist', block_access.actionAccessMiddleware("role", "read"), function (req, res) {
    ...
    filterDataTable("E_role", req.body, include).then(function (rawData) {
        entity_helper.prepareDatalistResult('e_role', rawData, req.session.lang_user).then(function (preparedData) {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("role", "read"), function (req, res) {
    ...
    models.E_role.findOne({
        ...
        e_role['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function (count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_role[subentityAlias].length; i++)
...
router.get('/show', block_access.actionAccessMiddleware("role", "read"), function (req, res) {
    ...
    entity_helper.optimizedFindOne('E_role', id_e_role, options).then(function (e_role) {
        if (!e_role) {
            data.error = 404;
        ...
        entity_helper.getPicturesBuffers(e_role, "e_role").then(function () {
            entity_helper.status.translate(e_role, attributes, req.session.lang_user);
            data.componentAddressConfig = component_helper.getMapsConfigIfComponentAddressExist("e_role");
...
router.get('/create_form', block_access.actionAccessMiddleware("role", "create"), function (req, res) {
    var data = {
        menu: "e_role",
    ...
    var view = req.query.ajax ? 'e_role/create_fields' : 'e_role/create';
    res.render(view, data);
});

router.post('/create', block_access.actionAccessMiddleware("role", "create"), function (req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_role.create(createObject).then(function (e_role) {
        var redirect = '/role/show?id=' + e_role.id;
        req.session.toastr = [{
            ...
            promises.push(new Promise(function (resolve, reject) {
                models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({where: {id: req.body.associationFlag}}).then(function (association) {
                    ...
                    if (typeof association['add' + modelName] !== 'undefined') {
                        association['add' + modelName](e_role.id).then(resolve).catch(function (err) {
                            reject(err);
                        });
                    } else {
                        var obj = {};
        ...
        model_builder.setAssocationManyValues(e_role, req.body, createObject, options).then(function () {
            Promise.all(promises).then(function () {
                component_helper.setAddressIfComponentExist(e_role, options, req.body).then(function () {
                    res.redirect(redirect);
                });
            }).catch(function (err) {
                entity_helper.error500(err, req, res, '/role/create_form');
            });
        });
    }).catch(function (err) {
        entity_helper.error500(err, req, res, '/role/create_form');
...
router.get('/update_form', block_access.actionAccessMiddleware("role", "update"), function (req, res) {
    var id_e_role = req.query.id;
    var data = {
        menu: "e_role",
        sub_menu: "list_e_role",
    ...
    entity_helper.optimizedFindOne('E_role', id_e_role, options).then(function (e_role) {
        ...
        data.e_role = e_role;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_role, "e_role", true).then(function () {
            if (req.query.ajax) {
                e_role.dataValues.enum_radio = data.enum_radio;
                res.render('e_role/update_fields', e_role.get({plain: true}));
            } else
...
router.post('/update', block_access.actionAccessMiddleware("role", "update"), function (req, res) {
    ...
    models.E_role.findOne({where: {id: id_e_role}}).then(function (e_role) {
        ...
        component_helper.updateAddressIfComponentExist(e_role, options, req.body);
        e_role.update(updateObject).then(function () {

            // We have to find value in req.body that are linked to an hasMany or belongsToMany association
            // because those values are not updated for now
            model_builder.setAssocationManyValues(e_role, req.body, updateObject, options).then(function () {
            ...
            entity_helper.error500(err, req, res, '/role/update_form?id=' + id_e_role);
        });
    }).catch(function (err) {
        entity_helper.error500(err, req, res, '/role/update_form?id=' + id_e_role);
    });
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('role', 'read'), function (req, res) {
    ...
    // Fetch tab data
    models.E_role.findOne(queryOpts).then(function (e_role) {
        if (!e_role)
            return res.status(404).end();

        var dustData = e_role[option.as] || null;
...
router.get('/set_status/:id_role/:status/:id_new_status', block_access.actionAccessMiddleware("role", "update"), function (req, res) {
    var historyModel = 'E_history_e_role_' + req.params.status;
    var historyAlias = 'r_history_' + req.params.status.substring(2);
    var statusAlias = 'r_' + req.params.status.substring(2);

    var errorRedirect = '/role/show?id=' + req.params.id_role;

    var includeTree = entity_helper.status.generateEntityInclude(models, 'e_role');
    ...
    models.E_role.findOne({
        where: {id: req.params.id_role},
        include: includeTree
    }).then(function (e_role) {
        if (!e_role || !e_role[historyAlias] || !e_role[historyAlias][0][statusAlias]) {
        ...
        models.E_status.findOne({
            where: {
                id: e_role[historyAlias][0][statusAlias].id
            },
            include: [{
                model: models.E_status,
                as: 'r_children',
            ...
            nextStatus.executeActions(e_role).then(function () {
                // Create history record for this status field
                // Beeing the most recent history for role it will now be its current status
                var createObject = {}
                createObject["fk_id_status_" + nextStatus.f_field.substring(2)] = nextStatus.id;
                createObject["fk_id_role_history_" + req.params.status.substring(2)] = req.params.id_role;
                models[historyModel].create(createObject).then(function () {
                    e_role['set' + entity_helper.capitalizeFirstLetter(statusAlias)](nextStatus.id);
                    res.redirect('/role/show?id=' + req.params.id_role)
                });
            }).catch(function (err) {
                console.error(err);
                req.session.toastr = [{
...
router.post('/search', block_access.actionAccessMiddleware('role', 'read'), function (req, res) {
    ...
    models.E_role.findAndCountAll(where).then(function (results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("role", "delete"), function (req, res) {
    ...
    models.E_role.findOne({where: {id: idEntity}}).then(function (e_role) {
        ...
        e_role['get' + entity_helper.capitalizeFirstLetter(alias)]().then(function (aliasEntities) {
            ...
            e_role['set' + entity_helper.capitalizeFirstLetter(alias)](aliasEntities).then(function () {
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("role", "create"), function (req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_role.findOne({where: {id: idEntity}}).then(function (e_role) {
        if (!e_role) {
        ...
        e_role['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function () {
...
router.post('/delete', block_access.actionAccessMiddleware("role", "delete"), function (req, res) {
    ...
    models.E_role.findOne({where: {id: id_e_role}}).then(function (deleteObject) {
        models.E_role.destroy({
            ...
            entity_helper.error500(err, req, res, '/role/list');
        });
    }).catch(function (err) {
        entity_helper.error500(err, req, res, '/role/list');
    });
