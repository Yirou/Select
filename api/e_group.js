...
router.get('/', function(req, res) {
    ...
    models.E_group.findAndCountAll(query).then(function(e_groups) {
...
router.get('/:id', function(req, res) {
    ...
    var query = {limit: answer.limit, offset: answer.offset, where: {id: id_e_group}};
    if (include.length)
        query.include = include;

    models.E_group.findOne(query).then(function(e_group) {
        if (!e_group) {
    ...
    models.E_group.create(createObject).then(function(e_group) {
        ...
        for (var prop in req.body)
            if (prop.indexOf('r_') == 0) {
                if (e_group['set'+entity_helper.capitalizeFirstLetter(prop)] !== 'undefined')
                    associationPromises.push(e_group['set'+entity_helper.capitalizeFirstLetter(prop)](req.body[prop]));
                else
                    console.error("API: Couldn't set association.\nAPI: e_group.set"+entity_helper.capitalizeFirstLetter(prop)+"() is undefined.");
            }
