...
router.get('/', function(req, res) {
    ...
    models.E_service.findAndCountAll(query).then(function(e_services) {
        answer["e_services".substring(2)] = e_services.rows || [];
        answer.totalCount = e_services.count;
        answer.rowsCount = answer["e_services".substring(2)].length;
...
router.get('/:id', function(req, res) {
    ...
    var id_e_service = parseInt(req.params.id);
    ...
    var query = {limit: answer.limit, offset: answer.offset, where: {id: id_e_service}};
    if (include.length)
        query.include = include;

    models.E_service.findOne(query).then(function(e_service) {
        if (!e_service) {
            answer.error = "No e_service with ID "+id_e_service;
            return res.status(404).json(answer);
        }
        answer["e_service".substring(2)] = e_service;
...
router.get('/:id/:association', function(req, res) {
    ...
    var id_e_service = req.params.id;
    ...
    models.E_service.findOne({
        ...
        answer[association] = e_service[include.as];
    ...
    models.E_service.create(createObject).then(function(e_service) {
        ...
        for (var prop in req.body)
            if (prop.indexOf('r_') == 0) {
                if (e_service['set'+entity_helper.capitalizeFirstLetter(prop)] !== 'undefined')
                    associationPromises.push(e_service['set'+entity_helper.capitalizeFirstLetter(prop)](req.body[prop]));
                else
                    console.error("API: Couldn't set association.\nAPI: e_service.set"+entity_helper.capitalizeFirstLetter(prop)+"() is undefined.");
            }
...
router.put('/:id', function(req, res) {
    ...
    models.E_service.findOne({where: {id: id_e_service}}).then(function(e_service) {
        ...
        // Update e_service
        e_service.update(updateObject, {where: {id: id_e_service}}).then(function() {
            answer["e_service".substring(2)] = e_service;
...
router.delete('/:id', function(req, res) {
    ...
    models.E_service.destroy({where: {id: id_e_service}}).then(function() {
        res.status(200).end();
    }).catch(function(err){
        answer.error = err;
        res.status(500).json(answer);
