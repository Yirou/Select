...
router.get('/', function(req, res) {
    ...
    models.E_activity.findAndCountAll(query).then(function(e_activitys) {
        answer["e_activitys".substring(2)] = e_activitys.rows || [];
        answer.totalCount = e_activitys.count;
        answer.rowsCount = answer["e_activitys".substring(2)].length;
...
router.get('/:id', function(req, res) {
    ...
    var id_e_activity = parseInt(req.params.id);
    ...
    var query = {limit: answer.limit, offset: answer.offset, where: {id: id_e_activity}};
    if (include.length)
        query.include = include;

    models.E_activity.findOne(query).then(function(e_activity) {
        if (!e_activity) {
            answer.error = "No e_activity with ID "+id_e_activity;
            return res.status(404).json(answer);
        }
        answer["e_activity".substring(2)] = e_activity;
...
router.get('/:id/:association', function(req, res) {
    ...
    var id_e_activity = req.params.id;
    ...
    models.E_activity.findOne({
        ...
        answer[association] = e_activity[include.as];
    ...
    models.E_activity.create(createObject).then(function(e_activity) {
        ...
        for (var prop in req.body)
            if (prop.indexOf('r_') == 0) {
                if (e_activity['set'+entity_helper.capitalizeFirstLetter(prop)] !== 'undefined')
                    associationPromises.push(e_activity['set'+entity_helper.capitalizeFirstLetter(prop)](req.body[prop]));
                else
                    console.error("API: Couldn't set association.\nAPI: e_activity.set"+entity_helper.capitalizeFirstLetter(prop)+"() is undefined.");
            }
...
router.put('/:id', function(req, res) {
    ...
    models.E_activity.findOne({where: {id: id_e_activity}}).then(function(e_activity) {
        ...
        // Update e_activity
        e_activity.update(updateObject, {where: {id: id_e_activity}}).then(function() {
            answer["e_activity".substring(2)] = e_activity;
...
router.delete('/:id', function(req, res) {
    ...
    models.E_activity.destroy({where: {id: id_e_activity}}).then(function() {
        res.status(200).end();
    }).catch(function(err){
        answer.error = err;
        res.status(500).json(answer);
